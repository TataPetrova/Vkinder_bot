...
class Vkinder:
    ...
    def test_params(self, user_ids, fields: str = "bdate, city, sex, relation"):  #Функция для получения параметров пользователя бота для подбора подходящей пары
        params = {
            "user_ids": user_ids,
            "fields": fields
        }
    ...
    def test_people(self, age_from, age_to, sex, city, status, offset, sorting: int = 0, count: int = 50): #Функция для получения списка людей по подходящим параметрам
        params = {
            "age_from": age_from,
            "age_to": age_to,
            "sex": sex,
    ...
    def test_photos(self, owner_id, album_id ="profile", photo_sizes = 1, extended = 1): #Функция для получения списка фотографий профиля и id выбранного пользователя
        params = {
            "owner_id": owner_id,
            "album_id": album_id,
